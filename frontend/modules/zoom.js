let mobile = require("modules/mobile"),
      ...
      if (srcData != "") {
              ...
              retObj.scales = {
                ...
                get mbasis() {
                  if (this.ownTOmain.W < 1 && this.ownTOmain.H < 1) {
                    ...
                    return this.ownTOmain.W < this.ownTOmain.H
                      ? "height"
                      : "width";
                  }
                },
        ...
        var centerImageOut = function() {
          ...
          var offset = {
            get left() {
              var metricLeft =
                (imageContainer.clientWidth - image.clientWidth) / 2;
              if (metricLeft < 0) return metricLeft;
              else return 0;
            },
            top: (imageContainer.clientHeight - image.clientHeight) / 2
          };
          if (left < 0) image.style.left = offset.left + "px";
          else image.style.left = 0 + "px";
          if (top < 0) image.style.top = offset.top + "px";
          else
            image.style.top =
              (imageContainer.clientHeight - image.clientHeight) / 2 + "px";
        };
        var retObj = {
          zoomStep: 5,
          swapZoomSteps: 0,
          ...
          calcProps: function() {
            if (this.scales.mbasis == "height") {
              image.style.width =
                parseInt(image.clientHeight / this.scales.own.kH, 10) + "px";
            } else if (this.scales.mbasis == "width") {
              image.style.height =
                parseInt(image.clientWidth / this.scales.own.kW, 10) + "px";
            }
          },
          get zoomScale() {
            var getter = this.scales.mbasis
              ? this.scales.mbasis
              : this.scales.basis;
          ...
          zoom: function(scale) {
            if (scale === undefined) {
              image.style[this.scales.mbasis] =
                this.scales.main[this.scales.mbasis] + "px";
              this.calcProps();
            }
            centerImageOut();
            if (mobile.status === true) {
          ...
          zoomIn: function(scale, max, force) {
            ...
            var limit = parseFloat(
              realScale / 100 * ZOOM_LIMIT_PERCENT + realScale
            );
        ...
        if (mobile.status === true) {
          ...
          if (window.hammer1 === undefined) {
            window.hammer1 = new Hammer(imageContainer, {
          ...
          var mouseHandlers = {
            ...
            move: function(e) {
              ...
              var diff = {
                x: retObj.mousePositions.x
                  ? retObj.mousePositions.x - retObj.mousePositions.lastX
                  : e.clientX,
                y: retObj.mousePositions.y
                  ? retObj.mousePositions.y - retObj.mousePositions.lastY
                  : e.clientY
              };
              ...
              if (posCheck.down === true && diff.x < 2 && diff.y < -2) {
                // down
                retObj.moveV(-vertical);
              }
              if (posCheck.up === true && diff.x < 2 && diff.y > 2) {
                // up
                retObj.moveV(vertical);
              }
              if (posCheck.right === true && diff.x > 2 && diff.y < 2) {
                // right
                retObj.moveH(horizontal);
              }
              if (posCheck.left === true && diff.x < -2 && diff.y < 2) {
                // left
                retObj.moveH(-horizontal);
              }
