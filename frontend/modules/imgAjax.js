let _ = require("underscore"), envConfig = require("../../config/env.json");
...
var imgAjax = {
  loadImages: function(element, callback) {
    ...
    var BASE_IMG_URL =
      protocol + "//" + envConfig.images_domain + "/get_image.cgi?product_id=";
    var images = element
      ? [element]
      : document.getElementsByClassName("-icecat-ajaxImg");
    ...
    for (var i = 0; i < images.length; i++) {
      var promise = new Promise(function(resolve, reject) {
        ...
        if (res[0] == "img") {
          if (res.length == 3) {
            url =
              BASE_IMG_URL + res[2] + "&type_data=json&type_image=" + res[1];
            el.classList.add("-icecat-hidden-pic");
          } else if (res.length == 4) {
            url =
              BASE_IMG_URL +
              res[2] +
              "&type_data=json&type_image=" +
              res[1] +
              "&product_gallery_id=" +
              res[3];
            if (res[1] !== "thumb") {
          ...
          url =
            protocol +
            "//" +
            envConfig.images_domain +
            "/get_image.cgi?product_bullet_id=" +
            res[1] +
            "&type_data=json";
        }
        ...
        if (url) {
          ...
          if (imgWidth) el.removeAttribute("width");
          if (imgHeight) el.removeAttribute("height");
