...
// ...almost normal code
function _3DTour(selector, options) {
  var defaultOptions = {
  ...
  var init = function (imgSrc) {
    imgSrc = imgSrc || (window.IcecatLive.initial3DTourResolution
      ? defaultOptions.sourceData[1][window.IcecatLive.initial3DTourResolution] 
      : defaultOptions.sourceData[1].Link400);
    var img = new Image();
    img.src = imgSrc;
    ...
    img.onload = function () {
      ...
      // and modify height
      defaultOptions.height = defaultOptions.currentHeight =
        options.width * imgProportion;
      ...
      var source = [];
      defaultOptions.sourceData = defaultOptions.sourceData.sort(function (
        a,
        b
      ) {
      ...
      var renderer = navigator.userAgent.match(/Firefox/i) ? "canvas" : "image"
        if (renderer === 'canvas') {
          initialRes = window.IcecatLive.initial3DTourResolution || 'Link1000'
        } else {
          initialRes = window.IcecatLive.initial3DTourResolution || 'Link400'
        }
      ...
      var spriteSpin = $(viewContainer).spritespin({
        ...
        onLoad: function () {
          /*
                         ...
                         */
          window.spritespinInstance = $(".spritespin-canvas").length > 0
            ? ".spritespin-canvas"
            : ".spritespin-stage";
  ...
  var self = this,
    ...
    zoom = {
      ...
      translate: [0, 0],
      zoomIn: function () {
        this.scale = this.scale >= this.maxZoom
          ? this.maxZoom
      ...
      getCurrentScaleFromInline: function (el) {
        ...
        var transformReg = /transform/, scaleReg = /scale\(([0-9.]+)\)/;
      ...
      getMaxOffset: function () {
        var offset = {};
        var max =
          (defaultOptions.currentWidth * this.scale / 2 -
            defaultOptions.currentWidth / 2) /
          this.scale;
        this.getMaxOffset.x = max;
        offset.x = max;
        max =
          (defaultOptions.currentHeight * this.scale / 2 -
            defaultOptions.currentHeight / 2) /
          this.scale;
      ...
      applyOffset: function () {
        ...
        /*
                     ...
                     */
        var transform =
          "scale(" + this.scale + ") translate3d(" + 0 + "px, " + 0 + "px,0)";
      ...
      closestPosition: function () {
        var x = this.translate[0],
          ...
          X = Math.abs(x) <= this.getMaxOffset.x
            ? x
            : sign.x * this.getMaxOffset.x,
          Y = Math.abs(y) <= this.getMaxOffset.y
            ? y
            : sign.y * this.getMaxOffset.y;
  ...
  self.resetHandlers = function () {
    for (var i in events) {
      (function (event) {
        var eventTarget = events[event].global
          ? $(window)
          : $(selector).find(events[event].selector);
  ...
  self.scrollHandler = function (e) {
    ...
    var containerWidth = $(".IcecatLive.for-tours3d").width();

   // if (e.pageX > containerWidth * 0.25 && e.pageX < containerWidth * 0.75) {
      e.preventDefault();
  ...
  self.moveStart = function (e) {
    if (
      e.pageX > window.innerWidth * 0.25 && e.pageX < window.innerWidth * 0.75
    ) {
      e.preventDefault();
...
window.SpriteSpin.extendApi({
  replaceSourceFrame: function (src, frame) {
    ...
    if (!this.replaceSourceFrame[src]) {
      var img = new Image();
      img.src = src;
      img.onload = function () {
        self.replaceSourceFrame[src] = true;
        window.SpriteSpin.create(self.data);
      };
    } else {
      window.SpriteSpin.create(self.data);
    }
  },
  betterQualityFrame: function (frame) {
    // Generic wi-fi signal is too slow for downloading of medium- and high-res frames
    if (isMobile) return;
    /*
             @TODO need to implement more flexible way for manipulating with canvas
             */

    frame = frame || this.data.frame;
    var oldSource = this.data.source[frame];

    var newSource = oldSource;
    this.data.sourceData.forEach(function (el) {
      if (el.OrderNumber == frame) {
        switch (oldSource) {
          case el.Link400:
            if (el.Link1000) {
              newSource = el.Link1000;
            }
            break;
          case el.Link1000:
            if (el.Link2000) {
              newSource = el.Link2000;
            }
            break;
        }
      }
    ...
    if (oldSource != newSource) {
      this.replaceSourceFrame(newSource);
    }
  },
  bestQualityRender: function () {
    // Generic wi-fi signal is too slow for downloading of medium- and high-res frames
    if (isMobile) return;
    ...
    this.data.sourceData.forEach(function (el) {
      var bestSource = el.Link2000 || el.Link1000 || el.link400;
      if (self.data.source[el.OrderNumber] != bestSource) {
        self.data.source[el.OrderNumber] = bestSource;
      }
    });
